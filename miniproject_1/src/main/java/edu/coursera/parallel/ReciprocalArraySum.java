...
public final class ReciprocalArraySum {
    ...
    public static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            if (this.endIndexExclusive - this.startIndexInclusive <= 1_000) {
                for (int i = this.startIndexInclusive; i < this.endIndexExclusive; i++) {
                    this.value += 1 / input[i];
                }
            } else {
                final int mid = (this.startIndexInclusive + this.endIndexExclusive) / 2;
                final ReciprocalArraySumTask left = new ReciprocalArraySumTask(this.startIndexInclusive, mid, this.input);
                final ReciprocalArraySumTask right = new ReciprocalArraySumTask(mid, this.endIndexExclusive, this.input);
                left.fork();
                right.compute();
                left.join();
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        for (int i = 0; i < numTasks; i++) {
            final ReciprocalArraySumTask task = new ReciprocalArraySumTask(
